...
//        assertEquals("This is Tomas", userController.getUser("Tomas"));
    ...
    @Test
    @DisplayName("Get a use by id not found exception")
    void getUser_exception() throws Exception{
        when(
                userService.getUser(UUID.fromString("79bf947e-a797-4ef4-923d-15cb8cd85cd6"))
        ).thenThrow(new UserNotFoundException("User not found"));

        mockMvc.perform(get("/user/{id}",UUID.fromString("79bf947e-a797-4ef4-923d-15cb8cd85cd6")))
                .andExpect(status().isNotFound())
                .andExpect(result -> assertTrue(result.getResolvedException() instanceof UserNotFoundException))
                .andExpect(result -> assertEquals(
                        "Ur not found", result.getResolvedException().getMessage()
                ));
    }
